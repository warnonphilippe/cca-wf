...
import be.civadis.wf.multitenancy.TenantUtils;
...
/**
 * Facade exposant les opérations courantes sur l'API activiti
 * Les droits d'accès dépendent de la logique métier de l'application, ses rôles et ses mécanismes de secu
 * Les vérifications doivent donc être effectuées en amont des appels à la facade qui n'est qu'une abstraction technique de l'API activiti
...
public class EngineFacade {
     ...
     *
     * REM : Nous ne définissons pas de user / group dans la secu d'activiti, nous utilisons les users et roles oauth2
     * Les roles étant plus fin que les groupes, associer des tâches activiti à des roles et non des groupes du user offre plus de souplesse
     *
     */
     ...
     * @param businessKey businessKey associée à l'instance
     * @param variables variables d'initialisation
     * @return
     */
    public ProcessInstance startProcess(String processName, String businessKey, Map<String, Object> variables){
        return runtimeService
            .createProcessInstanceByKey(processName)
            .processDefinitionTenantId(getCurrentTenant())
